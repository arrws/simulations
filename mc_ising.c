...
int delta_E(int fi,int fj) { return - 2 * Energy_function_for_one_spin(fi,fj); }

//flips spin (on the position: to_flip_i,to_flip_j)
void flip_spin(int i, int j) { spin[i][j] = - spin[i][j]; }
    ...
    for(step=0;step<N_steps_per_T;step++)
        {
            ...
            if (dE<=0) {
                ...
                if (r<p) flip_spin(to_flip_i,to_flip_j);
            ...
            if (t% N_write_per_T ==0) write_cmovie();
            t++;
        }
...
{
    printf("###########################################\n");
    printf("##  Monte Carlon Ising Model Simulation  ##\n");
    printf("###########################################\n\n");
    ...
    L_spin = 50;
    if (argc > 1) L_spin = atoi(argv[1]);
    if (argc > 2) moviefile = fopen(argv[2],"wb");
    ...
    spin_initialization();
    printf("\nRun started\n");
    printf("Timestep\tMagnetization\tchi\tEnergy\tCV\n");
